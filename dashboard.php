...
if(isset($_SESSION['activestaff']) || isset($_SESSION['activeadmin'])){
    ...
    if(isset($_REQUEST['tb1'])) {
        ...
        if(isset($_GET['tb1']) && $_GET['tb1']=="all"){
            ...
            $sql_forms = $db->query("SELECT * FROM product WHERE 1 and status = '1'");
            $pages->items_total = $sql_forms->rowCount();
            $pages->mid_range = 9;
            $pages->paginate();

            $result =   $db->query("SELECT * FROM product WHERE 1 and status = '1' ORDER BY id ASC ".$pages->limit."");

        }
        else if(isset($_GET['tb1']) && $_GET['tb1']!="all")
        {
            $condition      .=  " AND type='".$_GET['tb1']."'";
            //Main query
            $pages->default_ipp = 2;
            $sql_forms = $db->query("SELECT * FROM product WHERE 1 and status = '1'".$condition."");
            $pages->items_total = $sql_forms->rowCount();
            $pages->mid_range = 9;
            $pages->paginate();

            $result =   $db->query("SELECT * FROM product WHERE 1 and status = '1'".$condition." ORDER BY id ASC ".$pages->limit."");
        }
            ...
            <div class="d-flex justify-content-between">
                <div class="d-flex">
                    <form method="get" action="dashboard.php" class="form-inline">
                        ...
                        <label for="tab1" class="mr-2 font-weight-bold">Event Type:</label>
                        <select name="tb1" onchange="submit()" class="form-control">
                            <option value='all'>All</option>
                            <?php
                            $typeSql = "select type from product where status = '1' group by type order by type";
                            $type = $db->query($typeSql);
                            foreach($type as $m) {
                            if(isset($_GET['tb1']) && $_GET['tb1']==$m['type']){
                            ?>
                            <option value="<?php echo $m['type']?>" selected ><?php echo $m['type']?></option>
                            <?php } else {?>
                            <option value="<?php echo $m['type']?>"><?php echo $m['type']?></option>
                            <?php }}?>
                        </select>
                    </form>
                ...
                <div>
                    ...
                    <button class="btn btn-outline-primary ml-2 " onclick="window.document.location.href='addEvent.php'"><i class="fas fa-plus-square"></i></button>
                </div>
            </div>
...
function print(){
    ...
    var product = <?php echo json_encode($x); ?>;
    mywindow.document.write('<html><head><title>Organizili</title></br><h3 align=\'center\'>Events Report</h3><hr>');
    mywindow.document.write('</head><body><table style="width:100%"><tr><th>ID</th><th>type</th><th>choice</th><th>salle</th><th>Year</th></tr>');

    for (let index = 0; index < product.length; index++) {
        mywindow.document.write('<tr>');
        mywindow.document.write('<td align=\'center\'>'+product[index][0]+'</td>');
        mywindow.document.write('<td align=\'center\'>'+product[index][2]+'</td>');
        mywindow.document.write('<td align=\'center\'>'+product[index][1]+'</td>');
        mywindow.document.write('<td align=\'center\'>'+product[index][3]+'</td>');
        mywindow.document.write('<td align=\'center\'>'+product[index][4]+'</td>');
        mywindow.document.write('</tr>');
    }
